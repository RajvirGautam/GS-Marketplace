...
import Icons from '../../assets/icons/Icons'  // Correct: up to src, then into assets
import NeonButton from '../ui/NeonButton'     // Correct: same level components/ui
import AuthModal from '../modals/AuthModal'     // Correct: up one (layout) → components → modals
...
const Navbar = ({ isDark, toggleTheme }) => {
  ...
  useEffect(() => {
    const handleUpdate = () => {
      ...
      const maxScroll = 200
      let ratio = Math.min(scrollY / maxScroll, 1)
      ...
      const currentOpacity = 0.25 + 0.6 * ratio
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-bold text-indigo-900 dark:text-slate-200">
          {['Marketplace', 'Categories', 'About'].map((item) => (
            <a key={item} href="#" className="hover:text-fuchsia-600 dark:hover:text-cyan-400 transition-colors drop-shadow-sm">
              {item}
            </a>
          ))}
        ...
        {isOpen && (
          <div className="absolute top-[calc(100%+10px)] left-0 w-full p-4 rounded-2xl glass border border-white/20 flex flex-col gap-4 md:hidden animate-fade-in-up bg-white/60 dark:bg-black/80 backdrop-blur-2xl">
            {['Marketplace', 'Categories', 'About', 'Safety'].map((item) => (
              <a key={item} href="#" className="text-indigo-900 dark:text-white font-bold text-lg">
                {item}
              </a>
            ))}
